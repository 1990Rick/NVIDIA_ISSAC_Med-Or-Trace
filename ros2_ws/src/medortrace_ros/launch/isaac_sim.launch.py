...
``IsaacBackend.add_reset_hook(ros2_reset_hook(drive_from_cmd_vel=False))``: RTX lidar point cloud, RGB /
depth / camera_info, odometry, TF incl. the sensor frames, /clock from the simulation); the bridge node
then skips those topics, the backend keeps driving the wheels from the autonomy command (one controller),
the rig's static TF is not re-published, and the autonomy node refills the no-return rays of the RTX
cloud from the OR16 scan pattern.  Autonomy, operator console and gateway are the same nodes as in
simulation-lite.
    ...
    nodes = [
        ...
        Node(package="medortrace_ros", executable="autonomy_node", name="medortrace_autonomy", output="screen",
             parameters=[params, {**common, "mission_source": "topic", "audit_dir": L("audit_dir"),
                                  "lidar.fill_no_return_rays": graph_on, "lidar.pattern.rings": 16,
                                  "lidar.pattern.elev_min_deg": -15.0, "lidar.pattern.elev_max_deg": 15.0,
                                  "lidar.pattern.az_res_deg": 0.2}]),
