...
A variant is ``{"autonomy": {...}, "cfg": {...}, "sim": {...}}``: ``autonomy`` overrides the stack's
``autonomy`` block, ``cfg`` is merged into the whole scenario config (robot *and* world), ``sim`` is
merged into the simulator's config only (sim-to-real sensitivity, see ``runner.run_episode``).
Every row records its variant's non-empty overrides as ``variant_spec`` (``{}`` for a plain run; rows without
the key predate it) and, when used, ``sim`` / ``duration``, so every row can be re-run exactly
(``failure_atlas.reproduce_command``).
"""
...
def _run_one(entry: dict, policy: str, variant: str, autonomy_override: dict, cfg_override: dict,
             export_dir: str | None, duration: float | None, backend: str, save_raw: bool,
             sim_override: dict | None = None) -> dict:
    ...
    try:
        r = run_episode(cfg, e.seed, backend=backend, out_dir=out, policy=policy, duration=duration,
                        autonomy_override=autonomy_override, save_raw=save_raw, sim_overrides=sim_override or None)
    ...
    row = {"scenario_id": e.scenario_id, "family": e.family, "pair_id": e.pair_id, "split": e.split,
           "seed": e.seed, "policy": policy, "variant": variant, "hidden_cause": cfg.get("hidden_cause"),
           "cfg_hash": e.cfg_hash, "metrics": metrics, "error": err,
           "variant_spec": {k: v for k, v in (("autonomy", autonomy_override), ("cfg", cfg_override),
                                              ("sim", sim_override)) if v}}
    if sim_override:
        row["sim"] = sim_override
    if duration:
...
def run_batch(entries: list[RegistryEntry], policies=("active",), variants: dict | None = None,
    ...
    """``variants``: name -> {"autonomy": {...}, "cfg": {...}, "sim": {...}} (ablations / sensitivity)."""
